...
const clients = require('../db/controllers/clients');
const auth = require('../middleware');
router.post('/upsert', auth.checkToken(), async function(req, res, next) {
  let result = await clients.upsert(req.body);
  if (result !== 'SUCCESS') {
    res.status(500);
  }
...
router.get('/get', auth.checkToken(), async function(req, res, next) {
  let result = await clients.getAllData(req.query);
  if (result) {
    res.status(200);
  }
  res.send(result);
...
router.delete('/delete', auth.checkToken(), async function(req, res, next) {
  ...
  let result = await clients.deleteRecods(req.query);
  res.send(result);
  res.end();
});
